angular.module("NavbarApp").controller("NavbarCtrl", [
  "$rootScope",
  "$location",
  "$scope",
  "$state",
  ...
  function(
    ...
    $scope.showWhiteLogoFlag =
      $state.current.name === "LandingPage" || $state.current.name === "Market"
        ? true
        : false;
    ...
    if (
      $state.current.name === "LandingPage" ||
      $state.current.name === "About"
    ) {
    ...
    $scope.goToLogin = function() {
      ...
      ModalBuilderFct.buildComplexModal("md", view, controller, {}).then(
        function(response) {
          return;
        }
      );
    ...
    $scope.signup = function signup() {
      var modalDefaultsSignup = modalDefaults(
        "public/app/modules/accountapp/auth2app/views/signupmodal.view.html",
        "SignupFormCtrl"
      );

      ModalSvc.showModal(modalDefaultsSignup, {}).then(function(response) {});
    };
    ...
    $scope.goAccoutCreate = function() {
      ...
      /*
                    ...
                    applicable, data
                */
      var modalDefaultsAccountType = modalDefaults(
        authViews + "AccountTypeModal.html",
        "AccountTypeModalCtrl"
      );

      var modalDefaultsTenantSignup = modalDefaults(
        authViews + "AuthCreateAcctForm.html",
        "AuthCreateAcctModalCtrl",
        "Tenant"
      );

      var modalDefaultsPropertyManagerSignup = modalDefaults(
        authViews + "AuthCreateAcctForm.html",
        "AuthCreateAcctModalCtrl",
        "PropertyManager"
      );

      var modalDefaultsLogin = modalDefaults(
        authViews + "Login.html",
        "AuthLoginModalCtrl"
      );
      ...
      ModalSvc.showModal(modalDefaultsAccountType, {}).then(function(result) {
        ...
        if (result === "tenantSignup") {
          ModalSvc.showModal(modalDefaultsTenantSignup, {}).then(function(
            result
          ) {
          ...
          ModalSvc.showModal(modalDefaultsPropertyManagerSignup, {}).then(
            function(result) {
              if (result === "login") {
                ModalSvc.showModal(modalDefaultsLogin, {}).then(function(
                  result
                ) {
                  return;
                });
